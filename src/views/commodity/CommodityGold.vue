<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="价格" min-width="60px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="数量">
          <el-input v-model="temp.gold" type="number" placeholder="整数量" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getCommodityGoldList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addCommodityGold(tempData).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateCommodityGold(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$confirm('确定需要删除ID【' + row.id + '】吗？操作不可恢复!', '删除提示', {
        ...
        deleteCommodityGold({ id: row.id }).then(() => {
          this.$notify({
            title: 'Success',
