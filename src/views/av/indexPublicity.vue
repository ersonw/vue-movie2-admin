  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="标题" min-width="150px">
        <template slot-scope="{row}">
          <el-link :href="row.pic" target="_blank">
    ...
    <el-drawer size="60%" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 80%; margin-left:50px;">
        ...
        <el-form-item label="图片地址">
          <el-input v-model="temp.pic" type="textarea" />
        </el-form-item>
...
<script>
import { getIndexPublicity, deleteIndexPublicity, addIndexPublicity, updateIndexPublicity } from '@/api/av'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getIndexPublicity(this.listQuery).then(response => {
    ...
    handleDeleteAll() {
      deleteIndexPublicity({ 'ids': this.ids }).then(data => {
        this.$notify({
          title: 'Success',
          message: '删除成功',
          type: 'success',
    ...
    handleDelete(row, index) {
      var data = []
      data.push(row.id)
      deleteIndexPublicity({ 'ids': data }).then(data => {
        this.$notify({
          title: 'Success',
          message: '删除成功',
          type: 'success',
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addIndexPublicity(tempData).then((data) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.url1 = tempData.url
          // tempData.timestamp = +new Date(tempData.timestamp) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
          updateIndexPublicity(tempData).then((data) => {
            const index = this.list.findIndex(v => v.id === this.temp.id)
            const { result } = data
