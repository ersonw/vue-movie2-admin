<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="搜索配置名或注解" style="width: 20vw;margin-right: 1vw" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="配置名" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="配置值" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.val }}</span>
        </template>
      </el-table-column>
      <el-table-column label="注解" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.des }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="配置值">
          <el-input v-model="temp.val" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="配置值" />
        </el-form-item>
        <el-form-item label="注解">
          <el-input v-model="temp.des" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="注解" />
        </el-form-item>
      </el-form>
...
import { getAPPConfigList, updateAPPConfigList, updateConfig } from '@/api/APPConfig'
export default {
  // name: 'UsersList',
  ...
  methods: {
    ...
    getList() {
      ...
      getAPPConfigList(this.listQuery).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateAPPConfigList(tempData).then(() => {
