<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="展开" type="expand" width="100px">
        <template v-slot="slot">
          <el-table :data="slot.row.records">
            ...
            <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
              <template slot-scope="{row}">
                <el-button v-if="row.status!='deleted'" size="mini" type="danger" @click="handleDeleteRecord(row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
      ...
      </el-table-column><el-table-column label="小计" min-width="50px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="类名">
          <el-input v-model="temp.title" type="text" placeholder="类名" />
        </el-form-item>
        <el-form-item label="影视名">
          <span>{{ temp.records | recordsFilter }}</span>
          <br>
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="视频列表">
      ...
      <pagination v-show="pvTotal>0" :total="pvTotal" :page.sync="listQueryPv.page" :limit.sync="listQueryPv.limit" @pagination="getList" />
...
import {
  getBoutiqueList, deleteRecord, getList, addBoutique, updateBoutique, deleteBoutique
} from '@/api/videos'
export default {
  // name: 'UsersList',
  ...
  methods: {
    handleAddRecord(row) {
      if (row) {
        for (const record of this.temp.records) {
          if (record.vid === row.id) {
            this.$message({
              message: '相同视频已存在',
              type: 'error'
            })
    ...
    getList() {
      ...
      getBoutiqueList(this.listQuery).then(response => {
    ...
    handleDelete(row, index) {
      this.$confirm('确定需要删除分类【' + row.title + '】吗？操作不可恢复!', '删除提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteBoutique({ id: row.id }).then(() => {
          this.$notify({
            title: 'Success',
