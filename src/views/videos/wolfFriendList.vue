    ...
    >
      <el-table-column label="展开" type="expand" width="100px">
        <template v-slot="slot">
          <el-table :data="slot.row.recommends.list">
          ...
          <pagination v-show="slot.row.recommends.total>0" :total="slot.row.recommends.total" :page.sync="listQueryVideo.page" :limit.sync="listQueryVideo.limit" @pagination="getActorVideoList(slot.row.recommends)" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="人数" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.recommends.total }}</span>
        </template>
      </el-table-column>
...
import {
  ...
  getUnActorVideos,
  getWolfFriendList,
  removeWolfUser, getRecommendVideoList, deleteWolf
} from '@/api/videos'
let measurementsMap = []
export default {
  // name: 'UsersList',
  ...
  methods: {
    ...
    getList() {
      ...
      getWolfFriendList(this.listQuery).then(response => {
