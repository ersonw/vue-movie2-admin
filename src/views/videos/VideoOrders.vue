<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="视频ID" style="width: 20vw;margin-right: 1vw" class="filter-item" @keyup.enter.native="init" />
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" @click="init">
        搜索
      </el-button>
    ...
    >
      <el-table-column label="展开" type="expand" width="100px">
        <template v-slot="slot">
          <el-table :data="slot.row.orders.list">
          ...
          <pagination v-show="slot.row.orders.total>0" :total="slot.row.orders.total" :page.sync="listQueryVideo.page" :limit.sync="listQueryVideo.limit" @pagination="getActorVideoList(slot.row.orders)" />
        </template>
      </el-table-column>
      <el-table-column label="视频ID" prop="id" sortable="custom" align="center" width="80" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="订单数" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.orders.total }}</span>
        </template>
      </el-table-column>
...
import {
  ...
  updateActor,
  getUnActorVideos,
  getRecommendVideoList, getVideoOrdersList, removeOrderUser, deleteOrderVideo
} from '@/api/videos'
let measurementsMap = []
export default {
  // name: 'UsersList',
  ...
  methods: {
    ...
    getActorVideoList(videos) {
      getRecommendVideoList(this.listQueryVideo).then(res => {
    ...
    handleRemoveVideo(row) {
      removeOrderUser({ id: row.id }).then(() => {
        this.$message({
          message: '操作Success',
          type: 'success'
        })
    ...
    getList() {
      ...
      getVideoOrdersList(this.listQuery).then(response => {
    ...
    handleDelete(row, index) {
      this.$confirm('确定需要删除【' + row.title + '】并且清除所有关联数据吗？操作不可恢复!', '删除提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteOrderVideo({ id: row.id }).then(() => {
          this.$notify({
            title: 'Success',
