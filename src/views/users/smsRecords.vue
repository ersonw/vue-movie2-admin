<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="手机号" style="width: 20vw;margin-right: 1vw" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="验证码" min-width="50px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="手机号码" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.number }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发送时间" width="100px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="返回数据" min-width="50px">
        <template slot-scope="{row}">
          <span>{{ row.data }}</span>
        </template>
      </el-table-column>
...
import { del, getShare, getSmsRecordsList, getSuperior, update } from '@/api/users' // secondary package based on el-pagination
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getSmsRecordsList(this.listQuery).then(response => {
