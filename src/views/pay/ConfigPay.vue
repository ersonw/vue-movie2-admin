<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="搜索标题" style="width: 20vw;margin-right: 1vw" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="Api接口" min-width="300px">
        <template slot-scope="{row}">
          <span>{{ row.domain }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商户ID" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.mchId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商户秘钥" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.secretKey }}</span>
        </template>
      </el-table-column>
      <el-table-column label="同步通知地址" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.callbackUrl }}</span>
        </template>
      </el-table-column>
      <el-table-column label="异步通知地址" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.notifyUrl }}</span>
        </template>
      </el-table-column>
      <el-table-column label="错误返回地址" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.errorUrl }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getConfigPayList(this.listQuery).then(response => {
