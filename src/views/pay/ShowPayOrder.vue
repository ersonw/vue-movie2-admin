<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="接口ID" min-width="300px">
        <template slot-scope="{row}">
          <span>{{ row.orderId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="自平台ID" min-width="160px">
        <template slot-scope="{row}">
          <span>{{ row.orderNo }}</span>
        </template>
      </el-table-column>
      <el-table-column label="第三方平台ID" min-width="160px">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getShowPayOrderList(this.listQuery).then(response => {
    ...
    handleSuccess(row) {
      this.$confirm('确定需要手动修改平台ID【' + row.orderNo + '】为成功的状态吗？操作不可恢复!', '修改提示', {
        ...
        ShowPayOrderSuccess({ id: row.id }).then(() => {
          this.$notify({
            title: 'Success',
    ...
    handleFail(row) {
      this.$confirm('确定需要手动修改平台ID【' + row.orderNo + '】为失败的状态吗？操作不可恢复!', '修改提示', {
        ...
        ShowPayOrderFail({ id: row.id }).then(() => {
          this.$notify({
            title: 'Success',
            message: 'Delete Successfully',
            type: 'success',
