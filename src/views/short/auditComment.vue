<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="评论关键字" style="width: 300px;" class="filter-item" clearable @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="发布者" class-name="status-col" width="150px">
        <template slot-scope="{row}">
          <!--          <span class="link-type" @click="handleUser(row)">{{ row.nickname }}</span>-->
          <span class="link-type">{{ row.nickname }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发布IP" class-name="status-col" width="150px">
        <template slot-scope="{row}">
          <span>
            {{ row.ip }}
          </span>
      ...
      <el-table-column label="举报者" class-name="status-col" width="150px">
        <template slot-scope="{row}">
          <!--          <span class="link-type" @click="handleUser(row)">{{ row.nickname }}</span>-->
          <span class="link-type">{{ row.user }}</span>
        </template>
      </el-table-column>
      <el-table-column label="举报IP" class-name="status-col" width="150px">
        <template slot-scope="{row}">
          <span>
            {{ row.ipReport }}
          </span>
...
<script>
import { getAuditComment, deleteAuditComment, denyAuditComment, passAuditComment } from '@/api/shortVideo'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getAuditComment(this.listQuery).then(response => {
    ...
    handleDeleteAll() {
      deleteAuditComment({ 'ids': this.ids }).then(data => {
    ...
    handleDenyAll() {
      denyAuditComment({ 'ids': this.ids }).then(data => {
        const { list } = data
    ...
    handlePassAll() {
      passAuditComment({ 'ids': this.ids }).then(data => {
        const { list } = data
        for (let i = 0; i < list.length; i++) {
          const index = this.list.findIndex(v => v.id === list[i].id)
          this.list.splice(index, 1, list[i])
    ...
    handleDeny(row, index) {
      var data = []
      data.push(row.id)
      denyAuditComment({ 'ids': data }).then(data => {
        const { list } = data
    ...
    handlePass(row, index) {
      var data = []
      data.push(row.id)
      passAuditComment({ 'ids': data }).then(data => {
        const { list } = data
        this.list.splice(index, 1, list[0])
        this.$notify({
          title: 'Success',
    ...
    handleDelete(row, index) {
      var data = []
      data.push(row.id)
      deleteAuditComment({ 'ids': data }).then(data => {
        this.$notify({
          title: 'Success',
          message: '删除成功',
          type: 'success',
