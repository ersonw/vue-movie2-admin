<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="支付名称" width="150px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="支付代码" class-name="status-col" width="100">
        <template slot-scope="{row}">
          <el-tag type="success">
            {{ row.code }}
          </el-tag>
...
<script>
import { getOptionList, deleteOption, updateOption, addOption } from '@/api/cashIn'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getOptionList(this.listQuery).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateOption(tempData).then((data) => {
    ...
    handleDeleteAll() {
      deleteOption({ 'ids': this.ids }).then(data => {
        this.$notify({
          title: 'Success',
          message: '删除成功',
          type: 'success',
    ...
    handleDelete(row, index) {
      var data = []
      data.push(row.id)
      deleteOption({ 'ids': data }).then(data => {
        this.$notify({
          title: 'Success',
          message: '删除成功',
          type: 'success',
